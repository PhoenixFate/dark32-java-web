...
public class UserDaoImpl extends HibernateDaoSupport implements UserDao {
    ...
    public User getUserByCodePassword(final User user){
        User loginUser = getHibernateTemplate().execute(new HibernateCallback<User>() {
            public User doInHibernate(Session session) throws HibernateException {
                String hql = "from User where userCode=? and password=?";
                Query query = session.createQuery(hql);
                query.setParameter(0, user.getUserCode()).setParameter(1, user.getUserPassword());
                User user = (User) query.uniqueResult();
                return user;
            }
        });
