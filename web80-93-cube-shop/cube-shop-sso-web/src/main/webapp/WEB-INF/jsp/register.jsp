...
<div class="tabBox clear">
    ...
    <div class="login">已有账号，立即 <a href="${pageContext.request.contextPath}/user/login/page">登录</a></div>
</div>
<!--个人用户-->
...
<script type="text/javascript">
    var REGISTER = {
        ...
        beforeSubmit: function () {
            //检查用户是否已经被占用
            $.ajax({
                url: REGISTER.param.surl + "/user/username/" + escape($("#regName").val()) + "?r=" + Math.random(),
                success: function (data) {
                    console.log(data);
                    if (data.status===200) {
                        //检查手机号是否存在
                        $.ajax({
                            url: REGISTER.param.surl + "/user/phone/" + $("#phone").val() + "?r=" + Math.random(),
                            success: function (data) {
                                console.log(data);
                                if (data.status===200) {
                                    REGISTER.doSubmit();
        ...
        doSubmit: function () {
            $.post( REGISTER.param.surl + "/user/register", $("#regForm_mod").serialize(), function (data) {
                if (data.status === 200) {
