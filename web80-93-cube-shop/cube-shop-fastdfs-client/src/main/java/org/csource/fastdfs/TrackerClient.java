...
public class TrackerClient {
  private  static String[] storageAddressOld=new String[0];
  private  static String[] storageAddressNew=new String[0];
  private static Properties properties=new Properties();
  protected TrackerGroup tracker_group;
  protected byte errno;

  static {
    InputStream resourceAsStream = TrackerClient.class.getClassLoader().getResourceAsStream("myStorage.properties");
    System.out.println(resourceAsStream);
    try {
      if(resourceAsStream!=null){
        properties.load(resourceAsStream);
        storageAddressOld=properties.getProperty("storageAddressOld").split(",");
        storageAddressNew=properties.getProperty("storageAddressNew").split(",");
      }
    } catch (IOException e) {
      e.printStackTrace();
    }
   ...
   * constructor with global tracker group
   */
  public TrackerClient(String storageAddressOld,String storageAddressNew) {
    System.out.println("---hello---------------------------");
    this.storageAddressOld=storageAddressOld.split(",");
    this.storageAddressNew=storageAddressNew.split(",");
    this.tracker_group = ClientGlobal.g_tracker_group;
  }
  ...
  public StorageServer getStoreStorage(TrackerServer trackerServer, String groupName) throws IOException {
    ...
    try {
      ...
      String tempAddr=ip_addr+":"+port;
      System.out.println(tempAddr);

      for(int i=0;i<storageAddressOld.length;i++){
        if(tempAddr.equals(storageAddressOld[i])){
          System.out.println(storageAddressNew[i].split(":")[0]+":"+ Integer.parseInt(storageAddressNew[i].split(":")[1]));
          return new StorageServer(storageAddressNew[i].split(":")[0], Integer.parseInt(storageAddressNew[i].split(":")[1]), store_path);
        }
      }
